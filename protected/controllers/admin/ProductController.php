...
class ProductController extends AdminController {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Product'])) {
            ...
            $model->alias = MString::convertToAlias($_POST['Product']['name']);
            if ($model->save())
                echo '1';
            else
                echo '0';
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Product'])) {
            $model->attributes = $_POST['Product'];
            $menu = Menu::model()->findByPk($_POST['Product']['menuId']);
            $model->rootId = $menu->root;
            $model->alias = MString::convertToAlias($_POST['Product']['name']);
            if ($model->save()) {
                $keyword = Keyword::model()->findByAttributes(array('titleId' => $model->id));
                $keyword->model = Product::MODELPRODUCT;
                $keyword->title = $menu->name . ': ' . $model->name;
                $keyword->alias = 'san-pham/' . $model->id . '-' . $model->alias;
